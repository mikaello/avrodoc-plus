...
nunjucksEnv.addFilter('markdown', (value) => (value ? marked(String(value)) : ''));
...
function renderInlineType(type) {
    if (!type) return '';
    if (type.is_protocol) return '<a href="' + escHtml(type.link) + '">' + escHtml(type.protocol) + '</a>';
    if (type.is_record || type.is_enum || type.is_fixed) return '<a href="' + escHtml(type.link) + '">' + escHtml(type.name) + '</a>';
    if (type.is_array) return 'array&lt;' + renderInlineType(type.items) + '&gt;';
    if (type.is_map) return 'map&lt;' + renderInlineType(type.values) + '&gt;';
    if (type.is_union) return type.types.map(renderInlineType).join('&nbsp;|&nbsp;');
...
function minifiedJS(filename) {
    ...
    if (result.warnings && result.warnings.length > 0) {
        console.error('Warnings when minifying ' + filename + ':\n' + result.warnings);
    }
    return result.code;
}
...
function renderSections(ctx) {
    ...
    for (const [filename, schema] of Object.entries(ctx.schema_by_name)) {
        for (const [qualified_name, type] of Object.entries(schema.named_types)) {
            const route = '#/schema/' + encodeURIComponent(filename) + '/' + encodeURIComponent(qualified_name);
            const html = nunjucksEnv.render('named_type.njk', type);
            sections.push('<section data-route="' + escAttr(route) + '" hidden>\n' + html + '\n</section>');
        }
    }
...
async function topLevelHTML(title, extra_css_files, options) {
    const content = options.inline ? inlineContent(extra_css_files) : remoteContent(extra_css_files);
