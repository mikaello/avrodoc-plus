...
function parseAvroSchema(options, shared_types, schema_json, filename) {
    ...
    var avrodoc_custom_attributes = [
        ...
        'is_enum', 'is_message', 'is_record', 'is_protocol', 'is_error',
        'is_array', 'is_map', 'is_fixed', 'is_field', 'is_string', 'is_null',
        'is_int', 'is_long', 'is_double', 'is_union', 'is_primitive', 'attributes',
    ];
    ...
    function decorateCustomAttributes(schema) {
        ...
        if (schema.type !== null && hasOwnPropertyS(built_in_type_fields, schema.type)) {
            ignore_attributes = ignore_attributes.concat(built_in_type_fields[schema.type]);
        }
        for (var key in schema) {
            if (hasOwnPropertyS(schema, key) && ignore_attributes.indexOf(key) === -1) {
        ...
        allowedKeys.forEach(function (key) {
            var source =
                hasOwnPropertyS(field, key) && field[key] !== undefined
                    ? field
                    : isObject(field.type) && hasOwnPropertyS(field.type, key) && field.type[key] !== undefined
                      ? field.type
                      : null;
            if (!source) return;
            var val = source[key];
    ...
    function decorate(schema) {
        ...
        if (primitive_types.includes(schema.type)) {
            ...
            schema.link = ['#', 'schema', encodeURIComponent(filename || 'default'), encodeURIComponent(qualifiedName(schema))].join('/');
            schema.shared_link = ['#', 'schema', encodeURIComponent(qualifiedName(schema))].join('/');
        }
    ...
    function typeEssence(schema) {
        ...
        if (schema.type === 'record' || schema.type === 'error') {
            ...
            essence.errors = (schema.errors || []).map((error) => extractTypeName(error, schema.namespace));
            ...
            essence.messages = Object.fromEntries(
                Object.entries(schema.messages ?? {}).map(([messageName, message]) => [messageName, typeEssence(message)]),
            );
        } else {
    ...
    function isEqual(a, b) {
        ...
        if (a && b && typeof a == 'object' && typeof b == 'object') {
            ...
            for (i = length; i-- !== 0; ) if (!hasOwnPropertyS(b, keys[i])) return false;
            for (i = length; i-- !== 0; ) {
                var key = keys[i];
    ...
    function defineNamedType(schema, path) {
        ...
        if (hasOwnPropertyS(shared_types, qualified_name)) {
            shared_schema = shared_types[qualified_name].find((s) => isEqual(new_type, typeEssence(s)));
        } else {
        ...
        if (hasOwnPropertyS(named_types, qualified_name)) {
            ...
            if (!isEqual(new_type, existing_type)) {
                throw 'Conflicting definition for type ' + qualified_name + ' at ' + path + ': ' + JSON.stringify(existing_type) + ' != ' + JSON.stringify(new_type);
            }
        } else {
        ...
        if (schema == null) {
            ...
            if (schema.type === 'record' || schema.type === 'error') {
                ...
                schema.fields.forEach((field) => {
                    ...
                    field.table_annotations = buildTableAnnotations(field, options.annotationFields);
                });
    ...
    function parseProtocol(protocol) {
        ...
        protocol.types = (protocol.types ?? []).map((type) => parseSchema(type, protocol.namespace, 'types'));
        for (const [messageName, message] of Object.entries(protocol.messages ?? {})) {
            ...
            (message.request ?? []).forEach(function (param) {
                ...
                param.table_annotations = buildTableAnnotations(param, options.annotationFields);
            });
            ...
            message.errors = (message.errors ?? []).map((error) => parseSchema(error, protocol.namespace, joinPath(path, 'errors')));
            defineNamedType(message, path);
            decorate(message);
        }
        protocol.sorted_messages = Object.values(protocol.messages ?? {}).sort(stringCompareByS('name'));
    ...
    _public.sorted_types = Object.values(named_types).sort(stringCompareByS('name'));
...
export function buildAvroDocContext(input_schemata, options = {}) {
    const annotationFields = options.annotationFields || ['logicalType', 'aliases', 'order'];
    ...
    const namespaces = Object.values(namespacesMap)
        ...
        .map((ns) => ({
            ...
            types: [...ns.types].sort((a, b) => (a.name ?? '').localeCompare(b.name ?? '')),
        }));
    ...
    return { schema_by_name, shared_types, schemata, by_qualified_name, namespaces, protocols };
}
...
const hasOwnPropertyS = (object, property) => Object.prototype.hasOwnProperty.call(object, property);
